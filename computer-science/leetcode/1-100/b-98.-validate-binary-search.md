...
* **Space complexity:** $$O(N)$$. In the worst case, the depth of binary tree is $$N$$, which makes the recurison depth be $$O(N)$$.
...
[Inorder traversal](c-94.-binary-tree-inorder-traversal.md) means that each node's val should be smaller than the previous one's val if they are in a valid binary search tree.
